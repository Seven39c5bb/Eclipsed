    ...
    public virtual IEnumerator OnTurn()
    {
        ...
        for (int i = 0; i < originMobility; i++)
        {
            ...
            else
            {
                ...
                (int residualDistance, bool isMeleeAttack) = Move(nextDirection);
                float attackDelay = 0;
                if (isMeleeAttack)
                {
                ...
                float delay = 0.5f * (Mathf.Abs(nextDirection.x) + Mathf.Abs(nextDirection.y) - residualDistance) + attackDelay + 0.3f;
