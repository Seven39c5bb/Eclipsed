...
public class SalerDialogue : UIBase
{
    ...
    void Awake()
    {
        Register("SalerImg").onClick = OnClickWitch;
        DialoguePanel = GameObject.Find("SalerDialougePanel").GetComponent<CanvasGroup>();
        textLabel = GameObject.Find("SalerDialouge").GetComponent<TextMeshProUGUI>();

        // 初始化对话文本
        originDialogues.Add("给我金币，我能给你帮助。");
        originDialogues.Add("如果你有指骨，可以从我这里换到好东西，但我的要价会越来越高。");
        originDialogues.Add("小心一个披着红色斗篷、和我长得很像的家伙......想知道为什么？到下个地方再告诉你。");
        if(SaveManager.instance.jsonData.playerData.coin >= 300) originDialogues.Add("别把钱带进棺材里。");
        if(SaveManager.instance.jsonData.playerData.coin >= 200) originDialogues.Add("真是富有啊。");
        if(SaveManager.instance.jsonData.playerData.coin < 100) originDialogues.Add("再去赚点吧。");
